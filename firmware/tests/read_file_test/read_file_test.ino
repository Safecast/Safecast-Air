...
void setup()
{
    ...
    Serial.println("open new log file");
    bool done = false;
    while (!done)
    {
        Serial.print("Trying log file: ");
        Serial.println(openlog.eepromGetLogCount());
        done = openlog.openNewLogFile();
        if (!done)
        {
            openlog.eepromIncrLogCount();
            delay(1000);
        }
    }
    Serial.println("done");
